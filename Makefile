export COMPOSE_PROJECT_NAME=neuronation
...
create-database:
ifeq ($(INSIDE_DOCKER_CONTAINER), 0)
	@HOST_UID=$(HOST_UID) HOST_GID=$(HOST_GID) docker-compose $(PROJECT_NAME) exec mysql bash -c "mysql -uroot -p'secret' -e 'CREATE DATABASE IF NOT EXISTS laravel;'"
else
	$(ERROR_ONLY_FOR_HOST)
endif
...
drop-database:
ifeq ($(INSIDE_DOCKER_CONTAINER), 0)
	@HOST_UID=$(HOST_UID) HOST_GID=$(HOST_GID) docker-compose $(PROJECT_NAME) exec mysql bash -c "mysql -uroot -p'secret' -e 'DROP DATABASE IF EXISTS laravel;'"
else
	$(ERROR_ONLY_FOR_HOST)
endif
